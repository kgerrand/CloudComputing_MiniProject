...
def prepare_to_plot(full_data_dict, samples):
    """
    Prepares the data for plotting by concatenating the frames of the samples in the full_data_dict.
    Ensures that they're in the right order (in case of multiple workers mixing up the order)

    Args:
        full_data_dict (dict): A dictionary containing the data for each sample.

    Returns:
        dict: A dictionary containing the concatenated frames for each sample.
    """
    sorted_dict = {}

    # sorting the samples based on structure of samples.json
    for s in samples:
        frames = []
